const inputBox = document.getElementById("input-box"); //inp
const listContainer = document.getElementById("list-container"); //li
...
function updateCounter() {
  if (remaining === total) {
    ...
    circle.textContent = remaining + "/" + total;
  }
}
...
listContainer.addEventListener(
  ...
  function (e) {
    if (e.target.tagName === "LI") {
      ...
      updateCounter();
      saveData();
    } else if (e.target.closest(".bin-button")) {
      //SPAN is working like del button
      const listItem = e.target.closest("li");
      const wasChecked = listItem.classList.contains("checked");
...
function showTask() {
  ...
  remaining = parseInt(localStorage.getItem("remaining")) || 0;
  updateCounter();
}
showTask();
